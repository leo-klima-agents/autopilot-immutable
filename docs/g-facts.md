...
| G2 | Positions cannot be split by holders | **verified-in-published-code on the VE surface — but effectively splittable through the canonical VPM (fee-priced, governance-switchable)** | `core/VotingEscrow.sol` exposes no split: full mutating surface is `createStake`/`reviveStake`/`increaseStakeAmount`/`increaseStakingPeriod`/`withdraw`/`upgradeToPermanentStake`/`downgradeFromPermanentStake`/`rebalanceUnderlying`/`delegate` (lines 181–463). The only fractional carve-out is `rebalanceUnderlying`, role-gated — but per the G11 refinement, the canonical VPM makes it publicly reachable: one `withdrawToNFT(sourceId, destinations[])` call with N mint-sentinel legs splits a position into N fresh sAEROs (each inheriting the source's permanence/unlock, `core/VotingEscrow.sol:752-773`), for the pips fee, while `restricted` is off. Two structural limits keep it harmless to G1: the moved weight must sit idle on chain0 first (deallocation is itself cooldown-gated), so splitting never yields extra vote frequency per unit weight; and the brief already classified holder-splitting as benign ("tranche creation gets easier; design unaffected"). Competitive note: a whale can self-tranche this way — Model S territory; the small-user pooling case for Model P is untouched. `splitter/Splitter.sol` is a Minter team-fee splitter, unrelated to positions. |
...
| G11 | `withdraw-to-NFT` role-gated | **verified-in-published-code — role-gated as spec'd, but publicly reachable through the canonical VPM while governance leaves it open** | The carve-out primitive is `core/VotingEscrow.sol:349-430` `rebalanceUnderlying(SourceDelta[], DestinationDelta[])`: fractional drains from source positions, destinations may **mint new sTokens** (sentinel `tokenId = type(uint256).max`), balance-conserving, monotonic-unlock rule. Double-gated: caller must hold `VPM_ROLE` (`:354`, granted only by `VPM_ADMIN_ROLE`, `:52,149`) **and** be ERC-721-authorized for every source (`:358-364`). **Refinement (same-day re-read):** the protocol ships a canonical `VPM_ROLE` holder — `vpm/VoterPaymentsModule.sol` — whose `withdrawToNFT(sourceId, destinations[])` / `depositIntoNFT(sources[], destinationId, recipient)` (`:131-268`) are **public**: any caller that is ERC-721-authorized on the source (`_requireSourceAuth` → `VOTING_ESCROW.isAuthorized`, `:280-285`) may carve weight out to a freshly minted sAERO, paying a pips fee routed to the tokenId-0 accumulator. Two governance switches condition this: `restricted[selector]` (when on, only fee-registered callers; `setRestricted`, FEE_MANAGER) and the per-selector fee rate (default rate is a ceiling; `setFee`). So `exitToNFT` needs **no grant to the vault itself** — it needs the deployed VPM to keep `VPM_ROLE`, the selector unrestricted (or the vault registered), and a bearable live fee. **Design consequence: build `exitToNFT` as an opportunistic path through the VPM (read `getFee` live, tolerate `restricted` reverts, always fall back to share-selling); still ship Model S as the guarantee-free-of-governance fallback.** The protocol's own Relays exit holders through exactly this path (`relay/libraries/QueueLib.sol:196,559-561`), so it is load-bearing for them too — governance breaking it would break the official product first. |
...
4. **Model P's `exitToNFT` is an opportunistic VPM path, not a grant** (G11): the
   canonical VoterPaymentsModule exposes public `withdrawToNFT` for a governance-set
   fee unless governance restricts the selector — call it live, fall back to
   share-selling, and still ship Model S as the governance-independent fallback.
   The official Relays depend on the same path for their own exits.
