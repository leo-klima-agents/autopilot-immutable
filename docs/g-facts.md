...
| G11 | `withdraw-to-NFT` role-gated | **INVALIDATED — the primitive does not exist** | No fractional carve-out anywhere in `VotingEscrow.sol` (or any published contract). The spec-draft feature was dropped (at least from this batch). **Custody decision input: Model P can never ship `exitToNFT` against this code — pooled exit is shares-only. Model S remains fully viable: `sAERO` is a standard transferable ERC-721 (`ERC721('sAERO','sAERO')`, Enumerable, no soulbound gating), so whole-NFT exit works.** |
| G12 | `deposit-into-NFT` merge, owner-gated | **INVALIDATED — the primitive does not exist** | No merge in the published escrow. Deposits fall back to AERO + `createStake`/`increaseStakeAmount` — the path the brief pre-planned. |
...
| N4 | **VoterPaymentsModule (VPM)** can rebalance a token's allocation without moving the cooldown anchor (`Voter.sol:1274`) — but only if globally allowlisted **and** approved by the token's owner (`VotingEscrow.sol:497-499`). | Inert for the vault (it approves no one). Threat row closed by construction; assert in fork tests. |
...
distributor depends on is absent. What is already decided: **the custody trilemma resolves
against `exitToNFT`** (G11 dead) — ship Model P with shares-only exit, or Model S (whole-NFT,
fully viable). Begin writing TranchePilot's allocation layer against the exact-budget interface
now, but freeze nothing until a non-provisional tag with the rewards batch lands.
